...
export function TextField({ engine, path, label }) {
    const f = useFieldEditor(engine, path);
    const renders = useRef(0);
    renders.current++;
    return (_jsxs("div", { className: `field${f.diff ? ' changed' : ''}${f.error ? ' invalid' : ''}`, children: [_jsx("span", { className: "render-badge", title: "React renders", children: renders.current }), _jsx("label", { className: "field-key", children: label }), _jsx("input", { type: "text", className: "field-input", value: f.editing ? f.draft : f.value ?? '', onFocus: f.focus, onChange: (e) => f.change(e.target.value), onBlur: f.commit, onKeyDown: (e) => {
                    if (e.key === 'Enter')
                        e.target.blur();
                    if (e.key === 'Escape') {
                        f.cancel();
