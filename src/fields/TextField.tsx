...
export function TextField({ engine, path, label }: { engine: Engine; path: string; label: string }) {
  const f = useFieldEditor<string>(engine, path);
  ...
  return (
    <div className={`field${f.diff ? ' changed' : ''}${f.error ? ' invalid' : ''}`}>
      ...
      <input
        ...
        value={f.editing ? f.draft : f.value ?? ''}
        onFocus={f.focus}
